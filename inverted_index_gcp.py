...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        #TODO: we strated here
        self.docs_lengths = {}
        # set_of_total_tokens = set()
        # for doc_id, tokens in docs.items():
        #     for token in tokens:
        #         set_of_total_tokens.add(token)
        # self.docs_terms_weights_matrix = [[0] * len(set_of_total_tokens) for _ in range(len(docs))]

        for doc_id, tokens in docs.items():
            self.add_doc(doc_id, tokens)

    # #TODO: WE ADDED THIS FUNCTION
    # def _calculate_tf_idf(self):
    #     for term in self.term_total.keys():
    #         for doc_index in range(len(self.docs_terms_weights_matrix)):
    #             self.docs_terms_weights_matrix[doc_index][term] *= math.log10(len(self.docs_lengths) / self.df[term])
    ...
    def add_doc(self, doc_id, tokens):
        ...
        #TODO: WE ADDED THIS LINE
        self.docs_lengths[doc_id] = len(tokens)
        for w, cnt in w2cnt.items():
            self.df[w] = self.df.get(w, 0) + 1
            self._posting_list[w].append((doc_id, cnt))
            # #TODO: WE ADDED THIS LINE, SAVING TF IN THE MATRIX
            # self.docs_terms_weights_matrix[doc_id][w] = cnt
    ...
    @staticmethod
    def convert_tf_from_float_to_int(tf):
        return int(tf * (10**4))

    @staticmethod
    def convert_tf_from_int_to_float(tf):
        return tf / (10**4)
    ...
    def read_a_posting_list(self, base_dir, w, bucket_name=None):
        ...
        with closing(MultiFileReader(base_dir, bucket_name)) as reader:
            ...
            for i in range(self.df[w]):
                ...
                tf = InvertedIndex.convert_tf_from_int_to_float(tf)
                posting_list.append((doc_id, tf))
        return posting_list
    ...
    def write_a_posting_list(b_w_pl, base_dir, bucket_name=None):
        ...
        with closing(MultiFileWriter(base_dir, bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl: 
                #convert to int
                # convert to bytes
                b = b''.join([(doc_id << 16 | (InvertedIndex.convert_tf_from_float_to_int(tf) & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
                              for doc_id, tf in pl])
                # write to file(s)
                locs = writer.write(b)
